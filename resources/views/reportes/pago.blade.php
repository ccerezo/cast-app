...
<body>
    ...
    <table>
        ...
        <tfoot>
            <tr>
                <td colspan="3" class="text-right border" style="padding:10px 5px">Valor de la Factura #: {{ $factura->numero }}</td>
                <td class="text-right border" style="padding:10px 5px">$ {{number_format($factura->total,2)}}</td>
            </tr>
            @if(strcmp($factura->forma_pago, 'CREDITO') === 0)
            <tr>
                <td colspan="3" class="text-right border" style="padding:10px 5px">Total de Abonados Realizados</td>
                <td class="text-right border" style="padding:10px 5px">$ {{number_format($total_pagos,2)}}</td>
            </tr>
            @endif
        </tfoot>
    </table>
    <p class="text-center titulo">Pago Realizado</p>
    <table>
        <thead>
            <tr>
